...
<html>
  ...
  <body onLoad="setNavigation('/WmEntireX/directrpcadmin.dsp?adapterTypeName=WmEntireX', '/WmEntireX/WmEntireXAdapterHelp.html');">
	...
	<!-- IMPORTANT! Its READ BY IS (also if its HTML style commented) -->
	...
	<!-- %ifvar action equals('FipsMode')%
      	%invoke pub.wmentirex.directrpc.admin:setFipsMode%
      	%endinvoke%
	%endifvar%  -->
	...
	<table class="tableView">
		...
		<tr>
			...
			%ifvar tcpPortNumber1 -notempty%
			  %value tcpPortNumber1% %ifvar status matches('Running*')% listening %endif%
			%endif%
			</td>
			<script>writeTD('rowdata-l');swapRows();</script>
				%ifvar status matches('Stopped*')%
		...
		<tr>
			...
			%ifvar sslPortNumber1 -notempty%
				%value sslPortNumber1%  %ifvar status matches('Running*')% listening %endif%
			%endif%
			</td>
			<script>writeTD('rowdata-l');swapRows();</script>
				%ifvar status matches('Stopped*')%
		...
		<tr>
			...
			<script>writeTD('rowdata-l');swapRows();</script>
			%ifvar fipsMode equals('true')%
				<a href="directrpcadmin.dsp?adapterTypeName=WmEntireX&amp;action=FipsMode&amp;fipsMode=false">Disable</a>
			%else%
				<a href="directrpcadmin.dsp?adapterTypeName=WmEntireX&amp;action=FipsMode&amp;fipsMode=true">Enable</a>
			%endif%
			</td>
		</tr>
		...
		<TR>
			<TABLE class="tableView">
				...
				<TR>
					...
					<TD class="oddcol-l">Start Time</TD>
