...
function saveConnectionState()
{
	...
	if(status == "enable")
	{
		...
		window.parent.hideSub(false);	
		submitURLFromSubFrame('../WmSAP/ListResources.dsp?action=enableConnection&connectionState=enabled&adapterTypeName='+adapterTypeName+'&connectionAlias='+aliasName+'&dspName=.LISTRESOURCES&ISVersionGTE='+ISVersionGTE);
	
	}else
	{
		window.parent.hideSub(false);
		submitURLFromSubFrame('../WmSAP/ListResources.dsp?action=saveConnectionState&connectionState=disable&adapterTypeName='+adapterTypeName+'&connectionAlias='+aliasName+'&dspName=.LISTRESOURCES&ISVersionGTE='+ISVersionGTE);	
...
<input type="hidden" id="ISVersionGTE" value="%value ISVersionGTE%">
</body>
</html>
